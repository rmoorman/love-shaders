...
function loadShader()
   ...
   shader:send("blend", blendlayer)
   shader:send("refractionMap", geometry)
   shader:send("geometrySize", {geometry:getWidth(), geometry:getHeight()})
   shader:send("background", canvas)
   shader:send("refractionStrength", 60)
   shader:send("reflectionStrength", 200)
   shader:send("spectralAbberation", 0.02)
